%基于蚁群算法实现路径规划
function [Alocation,Newbest,traceInfo]=aca_ant_colony_system
%蚁群算法初始化程序开始
%问题：获取城市坐标和计算距离矩阵
InitOps=[];
...
%使用最近邻法构造一个初始游历，并据此计算信息素初值
p=zeros(1,Ncities+1);
p(1)=round(Ncities*rand+0.5);% P存储目前找到的所以城市编号
i=p(1);
count=2;
while count<=Ncities
    NNdist=inf;%NNdist 存储目前找到的和当前城市距离最短的城市路径
    pp=i;% i存储当前城市的编号，pp存储目前找到的城市编号
    for j=1:Ncities
        if (DistMatrix(i,j)<NNdist)&(j~=i)&((j~=p)==ones(1,length(p)))
            %目标城市的要求：距离短，不能为当前城市，且不能是以前走过的城市
...
Heuristic=1./DistMatrix;%启发信息矩阵
Temp=ones(1,Ncities);
Heuristic=1./(1./Heuristic+diag(Temp));
RandL=round(rand(M,1)*Ncities+0.5);%蚂蚁的初始位置
Alocation0=zeros(M,Ncities+1);%存放M+1个蚂蚁游历的路径以及长度矩阵初始化
Alocation0(:,1)=RandL;
...
Cbestdis=inf;
Fnewbest=0;
Alocation=Alocation0;%存放蚂蚁游历的路径以及长度矩阵初始化
Allow=Allow0;%允许矩阵赋初值
t1=clock;
%蚁群算法初始化程序结束，蚁群算法主循环开始
while(Nc<=MaxNc)
    %M个蚂蚁选择Ncitities个城市
    ...
    %计算每个蚂蚁找到的路径长度
    for Ak=1:M
        Alocation(Ak,:)=AcatspEval(Alocation(Ak,:),DistMatrix,Ncities);%计算适应值即每个蚂蚁找到的路劲长度
    end
    %保存最优，最差解
    t2=clock;
    t=etime(t2,t1);
    ...
    traceInfo(Nc,4)=std(Alocation(:,Ncities+1));%计算标准差
    ...
    Alocation=zeros(M,Ncities+1);%存放M+1个蚂蚁游历的路径及其长度矩阵初始化
    Alocation(:,1)=RandL;
...
grid on;
title(['全局最优解路线图-',num2str(Lbestdist)]);
xlabel('横坐标');
ylabel('纵坐标');
legend('城市位置');
hold off;
