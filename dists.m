...
if nargin==4 & ~isempty(e)
    ...
    if nargin<2||isempty(X2)
        m=0;% X2 default 
        if n<2
            error('X2 must have more than 1 point');
        end
    else
        [m,dX2]=size(X2);
        if m==0||~isreal(X2)
    ...
    elseif ischar(p) %Great circle distance
        p=lower(p);
        if d~=2
            error('Points must be 2-dimensional for great-circle distance');
        end
...
elseif n==1|m==0 %X1 only 1 point or intra-seq dist
        if n==1
            X1=X1(ones(1,m),:);%Expand X1 tomatch X2
            n=m;
        ...
        if p==2
            D=sqrt(sum(((X1-X2).^2+e)'));% Euclidean distance
        elseif ischar(p) %Great-circle distance
            X1=pi*X1/180;
            X2=pi*X2/180;
        ...
        elseif p==Inf&e==0
            D=max(abs(X1-X2)');% Chebychev distance
        else
            % otherwise
            D=zeros(1,n);
            for j=1:n
...
if ischar(p)& ~strcmp(p,'rad')
    if strcmp(p,'km')
        ...
        D=(3963.34 - 13.35*abs(sin(mean([X1(:,1);X2(:,1)]))))*D;%'mi'or 'sm'
    end
end
